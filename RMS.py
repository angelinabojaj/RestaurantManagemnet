...
background_frame = customtkinter.CTkFrame(managementUI, width=rect_width, height=rect_height, fg_color="#525fc7")
background_frame.grid(row=0, column=0, rowspan=8, sticky="nsw")
...
#Start 
    ...
    top_label = customtkinter.CTkLabel(top_frame, text="The Anything Coney Island", text_color="black", font=("Helvetica", 78)) 
    top_label.grid(row=0, column=2, pady=35, padx=180) #the pady and padx controls the height of the top rectangle box

    top_frame.grid_rowconfigure(0,weight=1)
    top_frame.grid_columnconfigure(0,weight=1)
    # Create a frame to act as the rectangle (background for buttons)
    background_frame = customtkinter.CTkFrame(employeeUI, width=rect_width, height=rect_height, fg_color="#525fc7")
    background_frame.grid(row=0, column=0, rowspan=8, sticky="nsw")
    ...
    def specialsButton_clicked():
        content_frame = customtkinter.CTkFrame(employeeUI,width=1350, fg_color="pink") 
        content_frame.grid(row=1, column=1, columnspan=8, sticky="nswe")

        specials_label = customtkinter.CTkLabel(content_frame, text="Today's Specials:\n1. Special Burger\n2. Vegan Tacos\n3. Will finish tomorrow", font=("Helvetica", 24), text_color="black")
        specials_label.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")  # Center the label within the frame
    
        # Ensure the row and column in content_frame can expand to fill the space
        content_frame.grid_rowconfigure(0, weight=1)
        content_frame.grid_columnconfigure(0, weight=1)

    logo = customtkinter.CTkButton(background_frame, text="RMS LOGO INSERT HERE", width=100, height=100)
    spacer = customtkinter.CTkLabel(background_frame, text="", width=rect_width, height= 100, fg_color="#525fc7")
    homeButton = customtkinter.CTkButton(background_frame, text="Home", width=200, height=100,command= homeButton_click)
    lookUpButton = customtkinter.CTkButton(background_frame, text="Look Up (Allergies)", width=200, height=100)
    startOrderButton = customtkinter.CTkButton(background_frame, text="Start Order", width=200, height=100)
    viewOrdersButton = customtkinter.CTkButton(background_frame, text="View Active Orders", width=200, height=100)
    tableManagementButton = customtkinter.CTkButton(background_frame, text="Table Management", width=200, height=100)
    DeliveriesButton = customtkinter.CTkButton(background_frame, text="Deliveries", width=200, height=100,command=specialsButton_clicked)
...
spacer = customtkinter.CTkLabel(background_frame, text="", width=rect_width, height= 100, fg_color="#525fc7")
homeButton = customtkinter.CTkButton(background_frame, text="Home", width=200, height=100,command= homeButton_click)
scheduleButton = customtkinter.CTkButton(background_frame, text="Schedule", width=200, height=100)
earningsButton = customtkinter.CTkButton(background_frame, text="Earnings", width=200, height=100)
transactionButton = customtkinter.CTkButton(background_frame, text="Transaction", width=200, height=100)
utilitiesButton = customtkinter.CTkButton(background_frame, text="Utilities", width=200, height=100)
TimeClockButton = customtkinter.CTkButton(background_frame, text="TimeClock", width=200, height=100)
